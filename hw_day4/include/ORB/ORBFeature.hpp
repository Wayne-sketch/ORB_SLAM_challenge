...
namespace ORB {
    class ORBFeature{
        ...
        private:
            std::shared_ptr<Parameter> camera_ptr; ///存相机参数
            cv::Mat image;  ///存要提取特征点的图像

            std::vector<double> mvScaleFactor, mvInvScaleFactor;///<每层图像的缩放因子 <以及每层缩放因子的倒数
            std::vector<double> mvLevelSigma2, mvInvLevelSigma2;///<存储每层的sigma^2,即上面每层图像相对于底层图像缩放倍数的平方 <sigma平方的倒数
            std::vector<cv::Mat> mvImagePyramid; //保存一张原图像所有层的金字塔图像
