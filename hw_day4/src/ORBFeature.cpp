...
namespace ORB {
    ...
    void ORBFeature::ExtractORB()
    {
        ...
        // 设置所有特征点的固定大小（例如，设置为10像素）
        for (cv::KeyPoint& keypoint : keypoints) {
            keypoint.size = 10.0; // 根据需要调整大小
        }

        // 以相同大小绘制特征点
        // 绘制特征点并显示图像
        cv::Mat image_with_keypoints;
        cv::drawKeypoints(image, keypoints, image_with_keypoints, cv::Scalar::all(-1),
                        cv::DrawMatchesFlags::DRAW_RICH_KEYPOINTS);

        // 显示图像
        cv::imshow("调用OpenCV函数提取的特征点", image_with_keypoints);
        cv::waitKey(0);
    }
    ...
    void ORBFeature::ExtractORB(std::vector<cv::KeyPoint>& _keypoints, cv::Mat& _descriptors)
    {
        ...
        for (int level = 0; level < camera_ptr->nLevels; ++level)
        ...
        {
            _descriptors = cv::Mat(nkeypoints, 32, CV_8U);//后面都直接用_descriptors处理
            // _descriptors.create(nkeypoints, 32, CV_8U); 不需要，_descriptors已经是Mat类型
//             descriptors = _descriptors; 不能用会报错
            // descriptors = _descriptors.clone(); 没测试
        }
...
static int bit_pattern_31_[256*4] =
{
    ...
    {
        //在ORB_SLAM2代码中，这些都是特征提取器的成员变量，在这里通过yaml文件读进camera_ptr指向的Parameter对象中，在这里读进来再使用
        int nlevels = camera_ptr->nLevels;
        int nfeatures = camera_ptr->nFeatures;
    ...
    //计算特征点的方向，返回角度作为方向 返回特征点的角度，范围为[0,360)，精度为0.3度]
    //image:要进行操作的某层金字塔图像 pt:当前特征点的坐标 u_max:图像块的每一行的坐标边界
    ...
    //计算ORB特征点的描述子
    //kpt:特征点对象 img:提取特征点的图像 pattern:预定义好的采样模板 desc:用作输出变量，保存计算好的描述子，维度为32x8=256bit
